...
import { z } from "zod";
import { Goal, Level, Equipment, Sex } from "@/generated/prisma/client";
...
export async function POST(req: Request) {
  ...
  const { goal, level, equipment, sex, age, weight, height, sessionsPerWeek } = parsed.data;
  ...
  // Menus nutritionnels
  const weekMenus = generateWeekMenus(targetCalories, goal);

  await prisma.$transaction(async (tx) => {
    // Profil utilisateur
    await tx.userProfile.upsert({
      where: { userId },
      create: { userId, goal, level, equipment, sex, age, weight, height, bmr, tdee, targetCalories, onboardingDone: true },
      update: { goal, level, equipment, sex, age, weight, height, bmr, tdee, targetCalories, onboardingDone: true },
    });
