...
const MUSCLE_BEGINNER: WorkoutTemplate = {
  name: "Prise de masse – Full Body Débutant",
  sessions: [
    {
...
const MUSCLE_INTERMEDIATE: WorkoutTemplate = {
  name: "Prise de masse – PPL Intermédiaire",
  sessions: [
    {
...
const FAT_LOSS_BEGINNER: WorkoutTemplate = {
  name: "Perte de gras – Cardio & Renforcement",
  sessions: [
    {
...
const FAT_LOSS_INTERMEDIATE = FAT_LOSS_BEGINNER; // même structure, intensité augmentée via poids

// ─── Rester en forme ──────────────────────────────────────────────────────────
const STAY_FIT_ANY: WorkoutTemplate = {
  name: "Maintien – Programme équilibré",
  sessions: [
    {
...
// ─── Reprise douce ────────────────────────────────────────────────────────────
const GENTLE_RETURN: WorkoutTemplate = {
  name: "Reprise douce – Programme progressif",
  sessions: [
    {
...
export function getWorkoutTemplate(
  ...
  if (goal === Goal.MUSCLE_GAIN) {
    template = level === Level.BEGINNER ? MUSCLE_BEGINNER : MUSCLE_INTERMEDIATE;
  } else if (goal === Goal.FAT_LOSS) {
    template = level === Level.ADVANCED ? FAT_LOSS_INTERMEDIATE : FAT_LOSS_BEGINNER;
  } else if (goal === Goal.GENTLE_RETURN) {
    template = GENTLE_RETURN;
  } else {
    template = STAY_FIT_ANY;
  }
